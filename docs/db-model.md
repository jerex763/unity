...
Constraint: `unique (person_id) where status != 'closed'` (partial) — Django: `UniqueConstraint(fields=['person'], condition=~Q(status='closed'), name='follow_up_one_open_per_person')`. Index: `(church_id, assigned_to_id, status)` — backs the core "my open follow-ups" view. Auto-creation rule (application logic): a Person still marked `visitor` must have exactly one open follow-up after their first actual check-in. If none is open, create `follow_up(source=event_visit, status=new)`; if one is already open, reuse it without changing its source, assignment or interaction history. Advance registration alone does not trigger the rule; walk-in registration does because it records attendance immediately.
