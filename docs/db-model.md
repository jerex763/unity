...
Constraint: `unique (person_id) where status != 'closed'` (partial) — Django: `UniqueConstraint(fields=['person'], condition=~Q(status='closed'), name='follow_up_one_open_per_person')`. Index: `(church_id, assigned_to_id, status)` — backs the core "my open follow-ups" view. Auto-creation rule (application logic): for a clean Person still marked `visitor`, first actual check-in ensures one open follow-up. If there is no open task and no historical `event_visit`, create `follow_up(source=event_visit, status=new)`; if a non-event follow-up is already open, reuse it without changing its source, assignment or interaction history. Any historical `event_visit`, including a closed record created by legacy registration-time behaviour, suppresses a new automatic task and is not migrated, deleted or reopened. Advance registration alone does not trigger the rule; walk-in registration does because it records attendance immediately.
